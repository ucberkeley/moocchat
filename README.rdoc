...
information can be easily extracted as JSON or CSV.
...
<tt>name, value</tt>::
  +event_name+ is a lower_snake_case string indicating what event
  happened.  A few events also have a string +event_value+, indicated here in
  parentheses; for events with no value, the +event_value+ column is blank:
...
0. +finish+  complete activity
0. +form_chat_group+  get placed in a chat group
0. <tt>continue(counter)</tt> learner clicks "Continue"; counter is which page in task (starting from 1 for first prologue page) she was viewing
0. <tt>proceed(counter)</tt> after clicking Continue and waiting for peers, learner moves on to next page.  Typically a +continue(n)+ event would be followed shortly after by <tt>proceed(n+1)</tt>.
0. <tt>user_data(key,new_data)</tt> Learner modifies user data in any way, even while still on page.  Since user data is a set of key-value pairs per user, if the learner modifies multiple data items, an event will be logged for **each** modified item.  
0. <tt>chat(utterance)</tt> say something during chat
