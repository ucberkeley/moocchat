...
sub run_test {
    ...
    $cmd = "$gHttperf --server=$gServer --method=POST --uri=/tasks/Learner+1/1/1 --num-conns=$num_tasks --rate=$rate --timeout=$timeout";
    warn "Running '$cmd'\n";
    $output = `$cmd`;
...
sub main {
    ...
    foreach $rate (@rates) {
        foreach $run (@runs) {
            @result = &run_test(20, $rate, $timeout);
